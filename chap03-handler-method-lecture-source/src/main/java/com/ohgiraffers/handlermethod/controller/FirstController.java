package com.ohgiraffers.handlermethod.controller;

import com.ohgiraffers.handlermethod.model.MenuDTO;
...
public class FirstController {
    ...
    /*
        컨트롤러 핸들러 메소드의 변환 값을 void로 설정하면 요청 주소가 view의 이름이 된다.
    ...
    public String registMenu(Model model, WebRequest request){
        ...
        String message = name + "이(가) 신규 메뉴 목록의 " + categoryCode + "번 카테고리에 " + price + "원으로 등록되었습니다.";
    ...
    /*
        ...
        이 경우 defaultValue를 이용하면 기본 값을 설정하여 사용할 수 있다.
     */
    ...
    public String modifyAll(Model model, @RequestParam(required = false) Map<String,String> parameters){
        ...
        String message = "메뉴의 이름이 " + modifyName + "(으)로, 가격이 " +modifyPrice+"원 으로 변경 되었습니다.";
    ...
    /*
        ...
        DTO 같은 커멘트 객체(요청 객체)로 전달받는다.

        @ModelAttribute의 경우 커멘드객체를 생성하여 매개변수로 전달해 준 뒤, 해당 인스턴스를 model에 담는다.
        화면에서 출력해보면 모델에 담겨진 값을 확인할 수 있다.

        경우에 따라 폼에서 입력한 값을 다음화면으로 바로 전달해야 하는 경우가 발생하는데 이 때 유용하게 사용할 수 있다.
     ...
     */
    @PostMapping("login1")
    public String sessionTest1(HttpSession session, String id){
    ...
    /*
        ...
        class 레벨에 @SessionAttributes 어노테이션을 선언하여 세션에 값을 담을 key값을 설정해두면
    ...
    /*
        @SessionAttributes로 등록된 세션은 session의 상태를 관리하는
        SessionStatus Object의 setComplete() 메소드를 호출하여 세션 만료가 가능하다.
        HttpSession보다 강력하게 관리하고 있는 것 같다.
     */
    @GetMapping("logout2")
    ...
    /*
        ...
        주로 RestAPI 작성 시 많이 사용 되며, 일반적인 form 전송을 항때에는 거의 사용하지 않는다.

        추가적으로
        헤더에 대한 정보를 @RequestHeader 어노테이션을 이용해서 가져올 수 있다.
        쿠키에 대한 정보를 @CookieValue 어노테이션을 이용해서 가져올 수 있다.
     */
